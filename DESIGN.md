---
version: alpha
name: FinTrack-design-analysis
description: An inspired modern interpretation of the FinTrack design language — combining an unusually heavy near-black display sans (weight 900 at 64–126 px) with a vivid lime-green brand accent, sage-tinted surface neutrals, and rounded white cards on a pale green-tinted canvas; the whole system reads like a clean, premium Scandinavian fintech magazine.
...
FinTrack wears its identity in a single signature pairing: a vivid lime-green `{colors.primary}` (`#9fe870`) used as the CTA pill and brand accent, set against a pale sage-tinted canvas `{colors.canvas-soft}` (`#e8ebe6`) that runs across the hero band, and a near-black ink `{colors.ink}` (`#0e0f0c`) with a hint of warmth from the brand's underlying olive cast. The brand reads like a calm Scandinavian magazine — generous whitespace, large rounded cards, and an unusually heavy display sans set at weight 900 carrying every hero headline.

Display typography is the second decisive voice. Bold geometric typography carries hero displays at weight 900 in scales from 64 px up to 126 px on the largest hero. The brand pairs weight 900 hero text with Inter at weight 600 for sub-displays — creating a high-impact hierarchy: weight 900 for the brand headline moment, weight 600 for section labels, and weight 400 for everything else.
...
- Two-tier display typography — Inter Display (weight 900, hero scale) + Inter (weight 600, sub-display scale). The contrast is the brand's typographic story.
...
- Statement summary card on the hero — hosting quick totals and primary action triggers.

## Colors

### Brand & Accent
- **FinTrack Green** (`{colors.primary}` — `#9fe870`): The brand's universal CTA color. Every primary button, every "Import CSV" pill, the brand's logo accent.
- **FinTrack Green Hover** (`{colors.primary-active}` — `#cdffad`): The lighter green for active state.
- **FinTrack Green Neutral** (`{colors.primary-neutral}` — `#c5edab`): A mid-saturation green used as a neutral active fill.
- **FinTrack Green Pale** (`{colors.primary-pale}` — `#e2f6d5`): The lightest green for soft surface tints / badge backgrounds.
...
1. **Inter (Display Weight 900)** — geometric sans with an unusually heavy weight 900 used for all hero displays. Always at weight 900, carrying the brand's typographic signature.
2. **Inter (Utility Weight 400 & 600)** — used for sub-displays (weight 600), body copy (weight 400), and form labels. Loaded with `font-feature-settings: "calt"` for contextual alternates.
...
### Principles
- **Weight 900 for hero, weight 600 for sub-headers, weight 400 for body.** Strict role separation.
...
- Background `{colors.ink}`, text `{colors.primary}` (FinTrack green!), padding `{spacing.xl}`, shape `{rounded.xl}`. Used for promotional moments.

**`currency-converter-card`** — the signature interactive widget.
...
- Background `{colors.canvas-soft}`, text `{colors.ink}`, padding `{spacing.3xl} {spacing.xl}`. Headline in `{typography.display-mega}` (weight 900).

**`hero-band-dark`** — the polarity-flipped dark hero.
- Background `{colors.ink}`, text `{colors.primary}` (FinTrack green headline on near-black!), same padding / scale.
...
### Do
- Reserve `{colors.primary}` FinTrack green for every primary CTA. The lime-green pill IS the brand's conversion signature.
...
- Use the full semantic palette (positive / warning / negative) for in-product status — never repurpose FinTrack green as success indicator since it IS the brand CTA.

### Don't
- Don't introduce a second brand accent. FinTrack green is the sole identity colour.
...
- Don't pair the green CTA with a green background. The brand always sits FinTrack green on neutral surfaces (sage / white / ink).
- Don't replace weight 900 with a lighter weight for hero typography — the heavy face IS the brand's voice.
